...
async function buscarDados() {
  ...
  try {
    // 1. Geocodificar endereço (non-fatal — name+number GeoSampa query works without it)
    let coordenadas = null;
    try {
      coordenadas = await geocodificarEndereco(endereco);
    } catch (e) {
      // Geocoding failed — we can still try GeoSampa by name+number
    }

    // 2. Buscar dados em paralelo
    const promises = [
      buscarDadosGeoSampa(endereco, coordenadas),
    ];
    ...
    // 3. Fetch owner/promisee data using the SQL code
    let proprietarioData = null;
    try {
      proprietarioData = await buscarProprietario(geoSampaData?.sql);
    } catch (e) {
      // Silently fail — owner data is non-critical
    }
...
// Remove accents from a string (GeoSampa stores "SAO" not "SÃO")
function removerAcentos(str) {
  return str.normalize('NFD').replace(/[\u0300-\u036f]/g, '');
}

// Extract street name keywords and number from user input
function extrairLogradouroNumero(endereco) {
  // Split on comma: "Rua Desembargador Carneiro Ribeiro, 763" -> parts
  const partes = endereco.split(',');
  const ruaPart = partes[0].trim();
  const numPart = partes.length > 1 ? partes[1].trim() : '';

  // Extract number: try after comma first, then from the street part
  ...
  // Remove accents, uppercase, strip street type prefix
  ...
  // Use last 2 words only — GeoSampa abbreviates titles (Desembargador→DESEM, Doutor→DR)
  // but never abbreviates the proper name at the end
...
async function buscarDadosGeoSampa(endereco, coordenadas) {
  // Run GeoSampa WFS and ViaCEP in parallel
  ...
  return {
    ...
    // Cadastral data from GeoSampa for IPTU
...
// Convert WGS84 lat/lon to UTM EPSG:31983 (SIRGAS 2000 / UTM zone 23S)
function latLonToUTM23S(lat, lon) {
  ...
  const ep2 = e2 / (1 - e2);
  const k0 = 0.9996;
  const lon0 = -45.0; // Central meridian for zone 23
...
// Make a WFS request to GeoSampa and return parsed features
...
// Query GeoSampa WFS for lot data — primary: by name+number, fallback: by coordinates
async function buscarLoteGeoSampa(endereco, coordenadas) {
  // Strategy 1: Search by street name + number (most accurate)
  ...
  // Strategy 2: Fallback to spatial query with coordinates (only if available)
  if (coordenadas) {
...
// Build IPTU data from GeoSampa results (no extra API call needed)
...
// Buscar proprietário e compromissário via portal da Prefeitura (sf8663)
async function buscarProprietario(sql) {
  if (!sql || sql === '-') {
    return { proprietario: '-', compromissario: '-' };
  }

  // Parse SQL "SSS.QQQ.LLLL-D" into components
  ...
  // Step 2: POST form with SQL components
  ...
  // Step 3: Extract names from response
...
function exibirResultados(dados) {
  ...
  // Show link to prefeitura portal when SQL is available
